...
public class NewBug implements Detector {
    ...
    public NewBug (BugReporter bugReporter) {
        ...
        for (Iterator<Location> i = cfg.locationIterator(); i.hasNext();) {
            ...
            try {
                ...
                if (upcast || typesAreEqual) {
                    if (!isCast) {
                        accumulator.accumulateBug(new BugInstance(this, "NEW_BUG_TYPE", NORMAL_PRIORITY)
                                        .addClassAndMethod(methodGen, sourceFile).addFoundAndExpectedType(refType, castType),
